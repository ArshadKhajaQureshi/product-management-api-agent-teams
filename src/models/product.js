...
/**
 * Validates and creates a batch of products atomically.
 * Two-phase: every item is validated and resolved into a plain array first (no
 * store mutation), then the store is mutated in a second pass — so a failure on
 * any item leaves the store untouched.
 * @param {Object[]} items - Product data objects (same fields as create()), each
 *   with sku additionally required to match SKU_FORMAT.
 * @returns {Object[]} The newly created products, in the same order as items.
 * @throws {ApiError} 422 if items is not a non-empty array, or any item fails field validation.
...
export const createMany = (items) => {
  ...
  const created = items.map((item) => {
    ...
    const { sku } = item;
    if (!SKU_FORMAT.test(sku)) {
      throw new ApiError(422, 'sku must match format /^[A-Z0-9-]{3,20}$/');
    }
    if (seenSkus.has(sku)) {
      throw new ApiError(409, `Duplicate sku "${sku}" within the request batch`);
    }
    seenSkus.add(sku);
    if (findBySku(sku)) {
