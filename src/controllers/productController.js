...
/**
 * Creates a batch of products atomically.
 * @param {import("express").Request} req - req.body holds { products: Object[] } (1-100 items).
 * @param {import("express").Response} res - Sends 201 with { created: number, products: Object[] }.
 * @param {import("express").NextFunction} next - Not used directly; errors are forwarded by catchAsync.
 * @returns {Promise<void>}
 * Status codes: 201 (created), 400 (products missing or not 1-100 items), 422 (an item fails field validation), 409 (a sku is duplicated in the batch or already exists).
 */
export const bulkCreateProducts = catchAsync(async (req, res) => {
  const { products } = req.body ?? {};
  const created = Product.createMany(products);
  res.status(201).json({ success: true, data: { created: created.length, products: created }, error: null });
});
