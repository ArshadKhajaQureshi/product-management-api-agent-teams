...
describe('POST /products/bulk', () => {
  const bulkProduct = (sku, overrides = {}) => ({
    name: `Bulk Product ${sku}`,
    sku,
    category: 'electronics',
    price: 9.99,
    stock: 5,
  ...
  it('returns 201 and creates every product in the batch', async () => {
    const res = await request(app)
      .post('/products/bulk')
      .send({
        products: [
          bulkProduct('BULK-001'),
          bulkProduct('BULK-002', { category: 'books' }),
          bulkProduct('BULK-003', { category: 'food' }),
        ],
      });
  ...
  it('rejects the entire batch when one product fails validation, and creates nothing', async () => {
    const res = await request(app)
      .post('/products/bulk')
      .send({
        products: [
          bulkProduct('BULK-004'),
          { sku: 'BULK-005', category: 'electronics', price: 9.99, stock: 5 },
        ],
      });
    ...
    const listRes = await request(app).get('/products');
    const skus = listRes.body.data.map((product) => product.sku);
    assert.ok(!skus.includes('BULK-004'));
    assert.ok(!skus.includes('BULK-005'));
    assert.equal(listRes.body.data.length, 2);
  });

  it('returns 409 when two products in the batch share a sku', async () => {
    const res = await request(app)
      .post('/products/bulk')
      .send({
        products: [bulkProduct('BULK-006'), bulkProduct('BULK-006', { category: 'books' })],
      });
  ...
  it('returns 409 when a batch sku already exists in the store', async () => {
    const res = await request(app)
      .post('/products/bulk')
      .send({ products: [bulkProduct(seeded.a.sku)] });
  ...
  it('returns 400 when the batch exceeds 100 items', async () => {
    const products = Array.from({ length: 101 }, (_, i) => bulkProduct(`BULK-${String(i).padStart(3, '0')}`));

    const res = await request(app).post('/products/bulk').send({ products });
